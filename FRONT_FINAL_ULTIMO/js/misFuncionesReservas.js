function autoInicioRelacionCliente(){
    ...
    $.ajax({
        url:"http://129.151.119.29:80/api/Client/all",
        type:"GET",
        datatype:"JSON",
...
function autoInicioGame(){
    ...
    $.ajax({
        url:"http://129.151.119.29:80/api/Game/all",
        type:"GET",
        datatype:"JSON",
        success:function(respuesta){
...
function agregarReservation() {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            url:"http://129.151.119.29:80/api/Reservation/save",
...
function listarReservation(){
    $.ajax({
        url:"http://129.151.119.29:80/api/Reservation/all",
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://129.151.119.29:80/api/Reservation/"+idElemento,
...
function cargarDatosReservation(id) {
    $.ajax({
        dataType: 'json',
        url:"http://129.151.119.29:80/api/Reservation/"+id,
...
function actualizarReservation(idElemento) {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://129.151.119.29:80/api/Reservation/update",
