function autoInicioCategoria(){
    ...
    $.ajax({
        url:"http://129.151.119.29:80/api/Category/all",
        type:"GET",
        datatype:"JSON",
...
function traerInformacionGame() {
    $.ajax({
        url:"129.151.119.29:80/api/Game/all",
...
function cargarDatosGame(id) {
    $.ajax({
        dataType: 'json',
        url:"http://129.151.119.29:80/api/Game/"+id,
    ...
    if($("#name2").val().length == 0 || $("#developer").val().length == 0 || $("#year").val().length == 0 || $("#description2").val().length == 0){
       alert("Todos los campos son obligatorios")
            ...
            $.ajax({
                ...
                url:"http://129.151.119.29:80/api/Game/save",
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://129.151.119.29:80/api/Game/"+idElemento,
...
function actualizar(idElemento) {
    ...
    if($("#name2").val().length == 0 || $("#developer").val().length == 0 || $("#year").val().length == 0 || $("#description2").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://129.151.119.29:80/api/Game/update",
